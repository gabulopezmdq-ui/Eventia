...
export async function GET(req: Request, props: { params: Promise<{ idEvento: string }> }) {
    try {
        ...
        // Extraer los search params de la URL entrante para pasarlos al backend
        const { searchParams } = new URL(req.url);
        const queryParams = new URLSearchParams(searchParams.toString());

        const res = await fetch(`${API_URL}/programas/${idEvento}/salud/panel?${queryParams.toString()}`, {
