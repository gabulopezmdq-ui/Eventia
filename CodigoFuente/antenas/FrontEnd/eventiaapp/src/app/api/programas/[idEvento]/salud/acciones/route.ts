...
export async function GET(req: Request, props: { params: Promise<{ idEvento: string }> }) {
    try {
        ...
        const res = await fetch(`${API_URL}/programas/${idEvento}/salud/acciones?${queryParams.toString()}`, {
        ...
        // Fase 5: Normalización del campo mixto idInscripcion
        const data = Array.isArray(rawData) ? rawData.map((item: any) => ({
            ...item,
            id_inscripcion: item.id_inscripcion || item.idInscripcion
        })) : rawData;
