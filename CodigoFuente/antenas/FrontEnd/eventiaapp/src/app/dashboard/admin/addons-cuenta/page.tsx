...
import { Briefcase, Loader2, RefreshCw, AlertTriangle, ShieldAlert, CheckCircle2, X } from 'lucide-react';
...
interface AddonPendiente {
    ...
    addon_codigo: string;
    addon_nombre: string;
    evento_anfitriones: string | null;
...
export default function AdminAddonsCuentaPage() {
    ...
    const fetchSolicitudes = useCallback(async () => {
        ...
        setError(null);
        try {
            const res = await fetch(`/api/admin/addons-cuenta/pendientes?mercado=${mercado}&moneda=${moneda}`);
            if (!res.ok) throw new Error('Error al cargar solicitudes pendientes de cuenta');
            const data = await res.json();
            setSolicitudes(data);
        } catch {
            setError('No se pudieron recuperar las solicitudes de cuenta pendientes.');
        } finally {
            setLoading(false);
        }
    }, [mercado, moneda]);
    ...
    const handleRegistrarPago = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            const res = await fetch('/api/admin/addons-cuenta/registrar', {
                method: 'POST',
            ...
            alert('¡Pago registrado con éxito! El add-on de la cuenta ahora está ACTIVO.');
            handleCloseModal();
            fetchSolicitudes();
        } catch (err) {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-200 dark:border-neutral-800 pb-6">
                <div>
                    <h1 className="text-2xl font-bold text-neutral-900 dark:text-white flex items-center gap-2">
                        ...
                        Cobranzas: Add-ons de Cuenta (B2B)
                    </h1>
                    <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
                        Visualizá y aprobá manualmente las solicitudes de add-ons de cuentas o agencias B2B.
                    </p>
                </div>
                            ...
                            <table className="w-full text-sm text-left">
                                <thead className="text-xs text-neutral-500 dark:text-neutral-400 uppercase bg-neutral-50 dark:bg-neutral-800/30">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 font-semibold">Cuenta B2B</th>
                    ...
                    <div className="w-full max-w-md bg-white dark:bg-neutral-900 rounded-3xl shadow-2xl border border-neutral-200 dark:border-neutral-800 overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="relative p-6 border-b border-neutral-100 dark:border-neutral-800">
                            ...
                            <h2 className="text-lg font-bold text-neutral-900 dark:text-white flex items-center gap-2">
                                ...
                                Registrar Pago de Add-on de Cuenta
                            </h2>
                            <p className="text-xs text-neutral-500 mt-1">
                                Confirmá el cobro para activar la funcionalidad en la cuenta de forma inmediata.
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleRegistrarPago} className="p-6 space-y-4">
                            ...
                            <div className="bg-neutral-50 dark:bg-neutral-800/30 p-4 rounded-2xl border border-neutral-100 dark:border-neutral-800 space-y-2">
                                <div className="flex justify-between text-xs">
                                    <span className="text-neutral-400">Cuenta B2B:</span>
                                    <span className="font-bold text-neutral-900 dark:text-white">{selectedRequest.cuenta_nombre}</span>
                                </div>
                                <div className="flex justify-between text-xs">
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="Ej: Transferencia - comp 000123"
