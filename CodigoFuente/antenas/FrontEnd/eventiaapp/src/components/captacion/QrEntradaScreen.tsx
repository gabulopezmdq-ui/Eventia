...
import { QrCode, CheckCircle2, XCircle, Search, AlertTriangle, AlertCircle, Camera, X, Loader2 } from 'lucide-react';
...
export default function QrEntradaScreen({ idEvento }: { idEvento: number }) {
    ...
    const handleScanQr = async (scannedToken: string) => {
        ...
        try {
            setLoading(true);
            setError(null);
            const data = await resolverQrEntrada(idEvento, scannedToken);
            setResult(data);
    ...
    // Camera scanner dynamic lifecycle
    useEffect(() => {
        ...
        if (scannerOpen) {
            const timer = setTimeout(async () => {
                try {
                    ...
                    const scanner = new Html5Qrcode("panel-entry-qr-reader");
                    ...
                    {!isManualSearch ? (
                        <>
                            ...
                            <button
                                ...
                                className="w-full py-3 mb-2 rounded-xl text-sm font-bold bg-indigo-600/10 border border-indigo-500/20 text-indigo-400 hover:bg-indigo-600/20 transition-all flex items-center justify-center gap-2 cursor-pointer"
                            >
                ...
                <div className="relative w-full max-w-md bg-card-bg border border-card-border rounded-3xl overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-200 z-10">
                    ...
                    <div className="flex items-center justify-between p-5 border-b border-card-border bg-card-bg">
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                <h4 className="text-sm font-bold text-foreground">Escáner de Entrada</h4>
                                <p className="text-muted text-[10px] uppercase font-semibold tracking-wider">Enfoque la entrada QR</p>
                            </div>
                        </div>
                        <button
                            onClick={stopScanner}
                    ...
                    <div className="relative aspect-square w-full bg-background flex items-center justify-center p-6 border-b border-card-border">
                        <div id="panel-entry-qr-reader" className="w-full h-full [&_video]:object-cover [&_video]:rounded-2xl" />
                        ...
                        {loading && (
                            <div className="absolute inset-0 bg-background/85 backdrop-blur-sm flex flex-col items-center justify-center text-center p-6 animate-in fade-in duration-200">
                                ...
                                <p className="text-sm font-bold text-foreground">Validando entrada...</p>
                    ...
                    <div className="p-5 bg-card-bg flex items-center justify-center">
                        <span className="text-[11px] font-semibold text-muted text-center max-w-[280px]">
                            Enfrente el código QR del pase del invitado a la cámara para validarlo de forma instantánea.
                        </span>
                    </div>
                </div>
            </div>
