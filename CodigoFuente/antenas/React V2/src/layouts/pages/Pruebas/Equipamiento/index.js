...
function Equipamiento() {
  ...
  useEffect(() => {
    axios
      .get("https://localhost:44382/tipoequipamiento/getall")
  ...
  const handleNuevoTipo = () => {
    navigate("/dashboards/Equipamiento/Nuevo");
  };
  ...
  const handleEdicion = (rowData) => {
    if (rowData && rowData.idTipoEquipamiento) {
      ...
      const url = `/dashboards/TipoEquipamiento/Edit/${productId}`;
          ...
          <DataTable
            table={{
              columns: [
                ...
                { Header: "Fecha Actualizado", accessor: "fechaAct" },
                {
                  Header: "Edit",
                  accessor: "edit",
                  Cell: ({ row }) => (
                    <Link
                      to={`/dashboards/TipoEquipamiento/Edit/${row.original.idTipoEquipamiento}`}
                    >
                      <MDButton
                        variant="gradient"
                        color="info"
