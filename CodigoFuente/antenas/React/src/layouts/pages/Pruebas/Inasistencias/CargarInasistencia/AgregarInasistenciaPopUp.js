...
const AgregarInasistenciaModal = ({ open, onClose, row, onConfirm }) => {
      ...
      const fetchOpciones = async () => {
        try {
          const response = await axios.get(
            `${process.env.REACT_APP_API_URL}inasistenciasCabecera/POF?dni=${row.dni}&legajo=${row.nroLegajo}`
          );
  ...
  const handleConfirm = () => {
    const payload = {
      idInasistenciaCabecera: row.idInasistenciaCabecera,
      pof,
      barra,
    };
    onConfirm(payload);
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      <DialogTitle>Agregar Inasistencia {row?.dni}</DialogTitle>
      <DialogContent>
        {/* Select POF */}
