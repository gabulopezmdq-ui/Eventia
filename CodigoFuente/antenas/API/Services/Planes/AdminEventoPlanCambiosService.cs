...
{
    public class AdminEventoPlanCambiosService : IAdminEventoPlanCambiosService
    {
        private readonly DataContext _context;

        public AdminEventoPlanCambiosService(DataContext context)
        {
            _context = context;
        }

        public async Task<List<AdminEventoPlanCambioItemDTO>> GetPendientesAsync()
        {
        ...
        public async Task<AdminEventoPlanCambioItemDTO> GetByIdAsync(long id_evento_plan_cambio)
        {
            var item = await GetQuery()
                .FirstOrDefaultAsync(x => x.id_evento_plan_cambio == id_evento_plan_cambio);
        ...
        public async Task<bool> AprobarAsync(AdminAprobarCambioPlanDTO req, long id_usuario_admin)
        {
            if (req == null)
                throw new Exception("Body inválido.");

            if (req.id_evento_plan_cambio <= 0)
                throw new Exception("id_evento_plan_cambio inválido.");

            var cambio = await _context.ef_evento_plan_cambios
                .FirstOrDefaultAsync(x => x.id_evento_plan_cambio == req.id_evento_plan_cambio);
            ...
            var evento = await _context.ef_eventos
                .FirstOrDefaultAsync(x => x.id_evento == cambio.id_evento);
            ...
            evento.id_plan = cambio.id_plan_solicitado;
            evento.fecha_modif = now;
            ...
            _context.ef_pagos.Add(new ef_pagos
            {
                ...
                concepto = $"Cambio de plan evento {cambio.id_evento} a {planSolicitado.codigo}",
                idempotency_key = $"CAMBIO_PLAN_EVT_{cambio.id_evento_plan_cambio}",

                objeto_tipo = "CAMBIO_PLAN_EVENTO",
        ...
        public async Task<bool> RechazarAsync(AdminRechazarCambioPlanDTO req, long id_usuario_admin)
        {
            if (req == null)
                throw new Exception("Body inválido.");
        ...
        private IQueryable<AdminEventoPlanCambioItemDTO> GetQuery()
        {
            return
                from c in _context.ef_evento_plan_cambios.AsNoTracking()
                join e in _context.ef_eventos.AsNoTracking()
                    on c.id_evento equals e.id_evento
                ...
                select new AdminEventoPlanCambioItemDTO
                {
                    id_evento_plan_cambio = c.id_evento_plan_cambio,
                    id_evento = c.id_evento,
                    evento_anfitriones = e.anfitriones_texto,
