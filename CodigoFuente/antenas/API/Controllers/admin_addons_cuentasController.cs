    ...
    public class admin_addons_cuentasController : ControllerBase
    {
        ...
        public async Task<IActionResult> Pendientes([FromQuery] string mercado = "AR", [FromQuery] string moneda = "ARS")
        {
            ...
            var items = await (
                ...
                join c in _context.Set<ef_cuentas>().AsNoTracking() on sa.id_cuenta equals c.id_cuenta
        ...
        public async Task<IActionResult> Registrar([FromBody] AdminRegistrarAddonPagoRequestDTO req)
        {
            ...
            if (sa.scope != "CUENTA") return BadRequest("La solicitud no es de CUENTA.");
            ...
            _context.Set<ef_pagos>().Add(new ef_pagos
            {
                ...
                impuestos = 0,
                total = req.importe,
                concepto = req.concepto ?? $"Pago manual addon {addon.codigo} - cuenta {sa.id_cuenta}",
